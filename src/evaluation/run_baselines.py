...
def run_baseline_evaluation(
    ...
    trivial_intents, trivial_actions = predict_trivial(messages, training_path)
    simple_intents, simple_actions = predict_simple(messages, training_path)
    ...
    for system_name, (predicted_intents, predicted_actions) in systems.items():
        results["systems"][system_name] = {
            "intent_metrics": intent_metrics(golden["intent"].tolist(), predicted_intents, list(INTENTS)),
            "action_metrics": action_metrics(golden["expected_action"].tolist(), predicted_actions),
        }
...
def main() -> None:
    ...
    predictions, results = run_baseline_evaluation(args.golden_set, args.training_data)
    current_results = Path("evaluation/results.json")
    if current_results.exists():
        results["existing_agent"] = json.loads(current_results.read_text(encoding="utf-8"))
    ...
    for system_name, system_results in results["systems"].items():
        print(
            f"{system_name}: intent accuracy={system_results['intent_metrics']['accuracy']:.4f}, "
            f"intent macro F1={system_results['intent_metrics']['macro_f1']:.4f}, "
            f"action accuracy={system_results['action_metrics']['accuracy']:.4f}, "
            f"action F1={system_results['action_metrics']['f1']:.4f}"
        )
    print("Created evaluation/baseline_predictions.csv and evaluation/baseline_results.json")
